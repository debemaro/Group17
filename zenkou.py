...
def create_random_block(x, y, add_to_space=True):
    ...
    if idx in [0, 1, 2]:   # 四角形
        ...
        return create_trapezoid(x, y, img, top=70, bottom=80, height=70, mass=70, add_to_space=add_to_space)
    elif idx in [6, 7]:   # 六角形
...
def draw_play():
    ...
    balls = []
    score = 0
    resume_button, title_button = None, None # ポーズボタン変数を初期化
    ...
    while True:
        ...
        # クリック前(カーソルxに連動、yは固定700)
        if not paused:
            mouse_x, _ = pygame.mouse.get_pos()
            #Pymunk座標系でBodyの位置を更新
